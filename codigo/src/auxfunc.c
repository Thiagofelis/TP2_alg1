...
int selectElement(int* vec, int size, int k)
{ // deletes vec in the process, so if we want to keep vec
  // we need to make a copy before calling this function
  // (we need to do this so we don't risk flooding the heap)
