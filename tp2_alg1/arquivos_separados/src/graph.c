...
{
    // assumes that the graph is complete
...
int graph_findBERecursive(graph* gp)
{
    ...
    edge_array *weights = graph_getWeigths(gp);
    ...
    *new_gp = graph_duplicateGraph(gp);
    ...
    // also possible that there's no edge smallest then the median. therefore, we need to check if
